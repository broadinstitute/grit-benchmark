...
def calculate_weighted_agg(
    ...
    def weighted_mean(groupby_df, weights='Metadata_grit'):
        solution = np.average(groupby_df.loc[:, features], 
                      axis=0, 
                      weights=groupby_df[weights])
        return solution
    ...
    if transform == 'softmax_grit':
        res_df = (population_df
              .groupby(columns)
              .apply(lambda x: weighted_sum(x, weights=weight))
             ).reset_index()
    ...
    # branch entered for mean weighted by grit 
        ...
        res_df = (population_df
            .groupby(columns)
            .apply(lambda x: weighted_mean(x, weights=weight))
         ).reset_index()

        res_df[population_df.loc[:, features].columns] = pd.DataFrame(np.vstack(res_df[0].values), columns=population_df.loc[:, features].columns)
