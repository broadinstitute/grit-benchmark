...
grit_df.loc[:, "gene"] = pd.Categorical(grit_df.gene, categories=grit_df.gene.unique())
...
sc_results_file = pathlib.Path(
    f"{perturbseq_data_dir}/{gse_id}_single_cell_grit.tsv.gz"
)
sc_df = pd.read_csv(sc_results_file, sep="\t")

sc_df.loc[:, "gene"] = pd.Categorical(sc_df.gene, categories=grit_df.gene.unique())
...
for gene in sc_df.gene.unique():
    ...
    gene_embedding_df.loc[
        gene_embedding_df.gene_identity == "neg", "grit_facet_label"
    ] = "Negative Ctrl"
